...
uint32_t Arr[5] = {0xA, 0xB, 0xC, 0xD, 0xE, 0xF};
...
{
	//enable HSE
	SET_BIT(RCC_BASE+RCC_CR_OFFSET,16);
	...
	//f_HSE = 8 MHz
	//
	//N = 84(我只把n改成一半 所以這部份註解的168都改成84)
	//M = 4
	//
	//f_VCO = 8 * 168 / 4 = 168 * 2
	//
	...
	WRITE_BITS(FLASH_BASE+FLASH_ACR_OFFSET,2,0,5);

	//use pll
	WRITE_BITS(RCC_BASE+RCC_CFGR_OFFSET,1,0,2);
